...
ProceduralTerrain::ProceduralTerrain(std::wstring heightmapFilename, std::string texFileName, std::string texFileName1, std::string texFileName2, GLuint program, Camera* camera, Light* light) {
...
void ProceduralTerrain::loadPerlinNoise() {
	...
	for (unsigned int i = 0; i < numCols; i++)
	{
		...
		{
			double x = (double)i / (double)numCols;
			double y = (double)j / (double)numRows;
			heightmap[i* numRows + j] = floor(255 * noise.GenerateNoise(x, y, 0.8));
		}
	}
}

void ProceduralTerrain::smooth() {
...
float ProceduralTerrain::average(UINT i, UINT j) {
...
float ProceduralTerrain::GetYPoint(Camera& camera)
{
	int z = static_cast<int>(camera.getCameraPosition().z);
	int x = static_cast<int>(camera.getCameraPosition().x);

	int move = (((numCols / 2) - z) * numRows) + x + (numRows / 2);

	if (move <= 0)
	{
		return vertices[0].pos.y;
	}
	else if (move >= vertices.size())
	{
		return vertices[vertices.size() - 1].pos.y;
	}

	return vertices[move].pos.y;
}

void ProceduralTerrain::createVerticesAndIndices() {
...
void ProceduralTerrain::buildBuffers() {
...
GLuint ProceduralTerrain::setTexture(std::string  texFileName) {
...
//void ProceduralTerrain::Render(RenderStruct & render)
//{
//
