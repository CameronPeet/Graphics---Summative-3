...
vec3 directionLightColor = vec3(1.0f, 0.0f, 0.0f);
vec3 pointLightColor = vec3(0.0f, 1.0f, 0.0f);
vec3 spotLightColor = vec3(0.0f, 0.0f, 1.0f);
...
{		
	...
	outColor += calcDirectionalLightColor();
	outColor += calcPointLightColor();  

	// If the light isn't on then just return 0 for diffuse and specular colors
	if (on == 1)
		outColor = calcSpotLightColor();

	vec3 totalColor = (ambient + outColor) * objectColor;
...
vec3 calcDirectionalLightColor()
{
	//**diffuse
	vec3 norm = normalize(Normal);
	vec3 lightDir = normalize(-lightDirection);  // negate => Must be a direction from fragment towards the light
	float diff = max(dot(norm, lightDir), 0.0);
	vec3 diffuse = diff * directionLightColor;
...
vec3 calcPointLightColor()
{
	...
	// Attenuation using Kc, Kl, Kq ---------------------------------------------------------
	float fdistance = length(lightPos - FragPos);  // distance to light
	float attenuation = 1.0f / (constant + linear * fdistance + exponent * (fdistance * fdistance));
